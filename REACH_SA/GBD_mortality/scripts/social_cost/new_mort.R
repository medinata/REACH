...
#' Baseline PM2.5 mortalities
base_PMdeaths <- read_csv(paste0(basemort_dir, 'basePM_mort.csv')) %>% 
...
compare_deaths <- left_join(all_deaths, base_PMdeaths, by = c("country", "cause_name")) %>%
  mutate(diff = new_mort - basePM_mort)
