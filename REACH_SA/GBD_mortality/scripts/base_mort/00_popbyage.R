# Description -------------------------------------------------------------
#' Organizes population data (under age 5 yr and above age 25 yr)
...
all_municip <- rbind(metro_municip, local_municip) %>%
  ...
  filter(start_age == 0 | start_age >= 25)
...
sa_pop <-  all_municip %>% 
  ...
  mutate(country = 'SOUTH AFRICA',
         age_group = ifelse(age_group == 'B04_2018', 'B0004_2018', age_group)) 
...
for (j in 1:nrow(pop_files)) {
  pop_list[[j]] <- readxl::read_excel(paste0(pop1_dir, pop_files$files[j]),
    ...
    dplyr::select(c(geo_name = GEO_CONCAT,country = CNTRY_NAME, B0004_2018, B2529_2018:B80PL_2018)) %>% 
    pivot_longer(cols = c(B0004_2018,B2529_2018:B80PL_2018), #only consider populations with ages of persons 25 + for concentration-response function
                 values_to = 'pop', 
                 names_to = 'age_group')
...
pop_info <- rbind(sa_pop, swz_pop, other_pop) %>% 
  mutate(age_group = case_when(!age_group %in% c('B80PL_2018', 'B75PL_2018') ~ # format the same as population files
         ...
         age_group = ifelse(age_group == '00-04 years', '<5 years', age_group)
  )
