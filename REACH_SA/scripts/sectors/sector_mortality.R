...
all_deaths <- rbind(other_deaths, stroke_deaths, IHD_deaths) %>% 
  ...
  mutate(new_mort = Tot_mort*adj_factor) %>% #ambient PM2.5 mortalities
  select(-Tot_mort)


base_deaths <- read_csv(paste0(basemort_dir, 'basePM_mort.csv')) %>% 
  select(country, base_mort = PM_mort, cause_name) 

left_join(all_deaths, base_deaths, by = c("country", "cause_name")) %>% 
  mutate(sector_mort = base_mort - new_mort,
