# Description -------------------------------------------------------------
#' Estimate NOx marginal social costs
#' Equations for marginal nitrate addition are from AP3
...
for (j in 1:nrow(poll_emis)) {
  ...
  tot_conc <- data.frame(Tot_HNO3 = do.call(rbind, poll_tot)) %>% 
    cbind(uncal_conc) %>% 
  ...
  molar_conc <- mutate(tot_conc,
    ...
    mutate(dHNO3 = HNO3_mol - Base_HNO3_mol,
           NO3_add1 = 0.564126*dHNO3,
           NO3_add2 = 0.311008*dHNO3,
