# Description -------------------------------------------------------------
#' Re-grid emissions (0.1x 0.1 degrees) to census geographies
...
inter_info <- read_csv(paste0(info_dir, 'inter_info.csv'))
...
for(j in 1:nrow(all_emis)) {
  ...
  emis_census <- left_join(inter_info, grid_df, by = 'grid_id') %>% 
    mutate(new_emis = emis*frac_of_grid) %>% 
    group_by(census_id) %>% 
    summarise(emis = sum(new_emis)) %>% 
    left_join(dplyr::select(domain_df, -census_area), by = "census_id") %>% 
  ...
  print(j/nrow(all_emis)*100) # print code progress
